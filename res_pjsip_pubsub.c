...
static struct ast_sip_subscription_handler *subscription_get_handler_from_rdata(pjsip_rx_data *rdata)
{
	...
	if (!event_header) {
		ast_log(LOG_ERROR, "Incoming SUBSCRIBE request with no Event header\n");
		return NULL;
	}
	...
	if (!handler) {
		ast_log(LOG_ERROR, "No registered subscribe handler for event %s\n", event);
	}
		...
		pj_status_t *dlg_status)
{
	...
	pjsip_dialog *dlg;
	struct subscription_persistence *persistence;
	ast_log(LOG_ERROR,"create_subscription_tree1\n");
	...
	ast_log(LOG_ERROR,"create_subscription_tree2\n");
	...
	ast_log(LOG_ERROR,"create_subscription_tree3\n");
	...
	ast_log(LOG_ERROR,"create_subscription_tree4\n");
	...
	ast_log(LOG_ERROR,"create_subscription_tree5\n");
...
static pj_bool_t pubsub_on_rx_notify_request(pjsip_rx_data *rdata)
{
	...
	ast_log(LOG_ERROR,"pubsub_on_rx_notify_request\n");
	pjsip_event_hdr *event_header;
	char event[32];
	RAII_VAR(struct ast_sip_endpoint *, endpoint, NULL, ao2_cleanup);
...
static pj_bool_t pubsub_on_rx_request(pjsip_rx_data *rdata)
{
	ast_log(LOG_ERROR,"pubsub_on_rx_request\n");
...
static void pubsub_on_rx_notify(pjsip_evsub *evsub, pjsip_rx_data *rdata, int *p_st_code,
		pj_str_t **p_st_text, pjsip_hdr *res_hdr, pjsip_msg_body **p_body)
{
	ast_log(LOG_ERROR,"pubsub_on_rx_notify\n");
