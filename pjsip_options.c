...
#ifdef GRANDSTREAM_NETWORKS
	/*add by wxu@grandstream.cn for peer trunk set PeerStatus*/
	enum ast_sip_contact_status_type peer_status = UNAVAILABLE;
...
#ifdef GRANDSTREAM_NETWORKS
	/*add by wxu@grandstream.cn for peer trunk set PeerStatus*/
	ast_log(LOG_DEBUG,"aor:%s",(endpoint_local != NULL? endpoint_local->aors:"endpoint is null"));
	if(endpoint_local != NULL && !strncmp(endpoint_local->aors,"trunk_",6)){
		manager_event(EVENT_FLAG_SYSTEM, "PeerStatus",
					"ChannelType: PJSIP\r\n"
					"Peer: PJSIP/%s\r\n"
					"PeerStatus: %s\r\n",
					(endpoint_local != NULL? endpoint_local->aors:""),
					(peer_status == AVAILABLE?"Reachable":"Unreachable"));
	}
	/*end*/
#endif
...
static int qualify_contact(struct ast_sip_endpoint *endpoint, struct ast_sip_contact *contact)
{
	...
	ast_log(LOG_ERROR, "contact = %d",contact->qualify_frequency);
	ast_log(LOG_ERROR, "---------%s\n", ast_sorcery_object_get_id(contact));
...
static int qualify_contact_task(void *obj)
{
	struct ast_sip_contact *contact = obj;
	int res;
	ast_log(LOG_ERROR, "++qualify_contact_task+++++%s\n", ast_sorcery_object_get_id(contact));
...
static int qualify_contact_sched(const void *obj)
{
	...
	ao2_ref(data->contact, +1);
	ast_log(LOG_ERROR, "++qualify_contact_sched+++++%s\n", ast_sorcery_object_get_id(data->contact));
...
{
	struct sched_data *data;
	ast_log(LOG_ERROR, "++schedule_qualify+++++%s\n", ast_sorcery_object_get_id(contact));
...
static void qualify_and_schedule(struct ast_sip_contact *contact)
{
	...
	ast_log(LOG_ERROR, "+++qualify_and_schedule+++++++%s\n", ast_sorcery_object_get_id(contact));
...
{
	ast_log(LOG_ERROR, "contact_created\n");
...
static pj_bool_t options_start(void)
{
	...
	ast_log(LOG_ERROR,"options_start\n");
...
static int qualify_and_schedule_cb(void *obj, void *arg, int flags)
{
	...
	ast_log(LOG_ERROR, "qualify_and_schedule_cb------%d\n",contact->qualify_frequency);
